...
void circuit::write_spef(const string &output)
{
  ...
  map<string, double> lumped_wire_cap_at_pin;              /* map from pin name -> cap */

  // 1. calc. lump sum caps from wires (PI2-model)
  for(vector<net>::iterator theNet = nets.begin() ; theNet != nets.end() ; ++theNet)
	{
		theNet->lumped_wire_cap = 0.0;
		if(theNet->name == clock_port)
			continue;
    ...
    {
			theNet->lumped_wire_cap += theSeg->second / static_cast<double>(DEFdist2Microns) * LOCAL_WIRE_CAP_PER_MICRON;
			lumped_wire_cap_at_pin [ theSeg->first.first ] += theSeg->second  / static_cast<double>(DEFdist2Microns) * LOCAL_WIRE_CAP_PER_MICRON * 0.5;
			lumped_wire_cap_at_pin [ theSeg->first.second ] += theSeg->second / static_cast<double>(DEFdist2Microns) * LOCAL_WIRE_CAP_PER_MICRON * 0.5;
      pin_cap_written [ theSeg->first.first ] = false;
      pin_cap_written [ theSeg->first.second ] = false;
    }
	}

  // 2. write wire parasitics 
  for(vector<net>::iterator theNet = nets.begin() ; theNet != nets.end() ; ++theNet)
  {
		// 0. write net name and lumped sum of wire cap 
		feed << "*D_NET "<< theNet->name << " " << theNet->lumped_wire_cap / CAP_SCALE << endl;
    ...
    for(vector<unsigned>::iterator theSink=theNet->sinks.begin() ; theSink != theNet->sinks.end() ; ++theSink)
        ...
        theSeg != theNet->wire_segs.end() ; ++theSeg)
		{
			...
			{
				feed << cnt++ << " " << theSeg->first.first << " " << lumped_wire_cap_at_pin[ theSeg->first.first ] / CAP_SCALE<<endl;
				pin_cap_written[ theSeg->first.first ]=true;
			}
			if(!pin_cap_written[ theSeg->first.second ])
			{
				feed << cnt++ << " " << theSeg->first.second << " " << lumped_wire_cap_at_pin[ theSeg->first.second ] / CAP_SCALE<<endl;
...
void circuit::write_timing(const string &output)
{
  ofstream dot_timing(output.c_str());
	...
	for(vector<unsigned>::iterator PI=PIs.begin() ; PI!=PIs.end() ; ++PI)
	{
		...
		feed << pins[ *PI ].rTran << " " << pins[ *PI ].fTran << " " << pins[ *PI ].rTran << " " << pins[ *PI ].fTran << endl;
	}
	...
	// 5. write load cap 
	for(vector<unsigned>::iterator PO=POs.begin() ; PO!=POs.end() ; ++PO)
		feed << "load " << pins[ *PO ].name << " " << pins[ *PO ].cap <<endl;
