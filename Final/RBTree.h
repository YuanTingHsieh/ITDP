...
// for contest
rb_red_blk_node* findFreeSites(rb_red_blk_tree* tree, void* key);
rb_red_blk_node* findDistTree(rb_red_blk_tree* tree, int* _size, int mode);
...
/***********************************************************************/
/* FUNCTION:  findFreeSite */
/* USAGE:  find the nearest site from given site index key */
/***********************************************************************/

rb_red_blk_node* findFreeSites(rb_red_blk_tree* tree, void* key) {
    rb_red_blk_node* overlappedSiteNode;
    rb_red_blk_node* predecessorSiteNode;
    rb_red_blk_node* successorSiteNode;
    rb_red_blk_node* freeSiteNode = (rb_red_blk_node*)malloc(sizeof(rb_red_blk_node));
	freeSiteNode->key = (int*)malloc(sizeof(int));
	freeSiteNode->info = (int*)malloc(sizeof(int));
	int* queryKey = (int*)malloc(sizeof(int));
	*queryKey = *(int*)key;
    overlappedSiteNode = RBTreeInsert(tree, queryKey, 0);
    predecessorSiteNode = TreePredecessor(tree, overlappedSiteNode);
    successorSiteNode = TreeSuccessor(tree, overlappedSiteNode);
	if(predecessorSiteNode == tree->nil && successorSiteNode == tree->nil)
		return NULL; // lose memory
	if(predecessorSiteNode == tree->nil) {
		*(int*)freeSiteNode->key = *(int*)successorSiteNode->key;
		*(int*)freeSiteNode->info = *(int*)successorSiteNode->info;
        RBDelete(tree, successorSiteNode);
		RBDelete(tree, overlappedSiteNode);
		return freeSiteNode;
	}
	if(successorSiteNode == tree->nil) {
		*(int*)freeSiteNode->key = *(int*)predecessorSiteNode->key;
		*(int*)freeSiteNode->info = *(int*)predecessorSiteNode->info;
        RBDelete(tree, predecessorSiteNode);
		RBDelete(tree, overlappedSiteNode);
		return freeSiteNode;
	}
    if(*(int*)overlappedSiteNode->key - *(int*)predecessorSiteNode->key <
       *(int*)successorSiteNode->key - *(int*)overlappedSiteNode->key) { // choose predecessor
        *(int*)freeSiteNode->key = *(int*)predecessorSiteNode->key;
		*(int*)freeSiteNode->info = *(int*)predecessorSiteNode->info;
        RBDelete(tree, predecessorSiteNode);
    }
...
// mode 0: return predecessor or self
// mode 1: return successor or self
rb_red_blk_node* findDistTree(rb_red_blk_tree* tree, int* _size, int mode) {
	//printf("file %s line %d\n", __FUNCTION__, __LINE__);
	rb_red_blk_node* resultNode;
	rb_red_blk_node* q;
	int* size = (int*)malloc(sizeof(int));
	*size = *_size;
	//printf("file %s line %d\n", __FUNCTION__, __LINE__);
	resultNode = RBExactQuery(tree, size);
	//printf("file %s line %d\n", __FUNCTION__, __LINE__);
	if(resultNode) {
		
		//printf("file %s line %d size = %d\n", __FUNCTION__, __LINE__, *size);
		//printf("resultNode->key = %d\n", *(int*)(resultNode->key));
		return resultNode;
	}
	else {
		//printf("file %s line %d\n", __FUNCTION__, __LINE__);
		q = RBTreeInsert(tree, size, 0);
		//printf("file %s line %d\n", __FUNCTION__, __LINE__);
		if(mode == 0)
			resultNode = TreePredecessor(tree, q);
		else
			resultNode = TreeSuccessor(tree, q);
		//printf("file %s line %d\n", __FUNCTION__, __LINE__);
		//printf("resultNode->key = %d\n", *(int*)(resultNode->key));
		RBDelete(tree, q);
		//printf("file %s line %d\n", __FUNCTION__, __LINE__);
