...
void circuit::read_verilog(const string &input)
{
  ...
  do
  {
    ...
    if (valid)
    {
      ...
      myPin->net    = net2id[myNet->name];
			myPin->type   = PI_PIN;
			myPin->portType = 2; // primary Input
  ...
  do
  {
    ...
    if (valid)
    {
      ...
      myPin->net   = net2id[myNet->name];
			myPin->type  = PO_PIN;
			myPin->portType = 3; // primary Output
  ...
  while (!dot_def.eof())
  {
    ...
    else if (tokens[0] == "ROW" && mode == INIT)
    {
      ...
      if (tokens[0] == "DO")
      {
		...
		myRow->siteIndexOffset = numSites;
		numSites += myRow->numSites;
		siteMappingWidth = max(myRow->numSites, siteMappingWidth);
